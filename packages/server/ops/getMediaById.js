...
export default function (id) {
  ...
  logger.debug(`found: id=${id} title="${media.title}" status=${media.status} type=${media.type}`);

  if (media.status === 'error') {
    logger.debug(`status=error, fetching job error for id=${id}`);
    const job = db.prepare(
      "SELECT error FROM jobs WHERE media_id = ? AND status = 'failed' ORDER BY updated_at DESC LIMIT 1"
    ).get(Number(id));
    if (job?.error) {
      logger.debug(`job error: ${job.error}`);
      media.job_error = job.error;
    } else {
      logger.debug(`no job error found for id=${id}`);
    }
  }
