...
const iconPaths: Record<string, { d: string; stroke?: boolean; fill?: boolean }> = {
...
export function Icon({ name, size = 20, color = colors.fg }: IconProps) {
  const icon = iconPaths[name];
  if (!icon) return null;

  return (
    <Svg width={size} height={size} viewBox="0 0 24 24" fill={icon.fill ? color : 'none'} stroke={icon.stroke ? color : undefined} strokeWidth={icon.stroke ? 2 : undefined} strokeLinecap="round" strokeLinejoin="round">
      <Path d={icon.d} />
    </Svg>
  );
}
